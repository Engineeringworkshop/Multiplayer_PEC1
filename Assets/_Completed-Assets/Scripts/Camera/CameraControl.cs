...
namespace Complete
{
    public class CameraControl : MonoBehaviour
    {
        ...
        [HideInInspector] public List<TankManager> m_ActiveTargets;
		...
		private void Split() {
			...
			float distance = CheckDistanceBetweenTanks();
            ...
            if (splitMode && distance < limitDist - hysteresis) {
                ...
                foreach (Camera tankCamera in Resources.FindObjectsOfTypeAll<Camera>())
                {
                    if (tankCamera.name.StartsWith ("Camera")) {
        ...
        // Method to calculate maximun distance between tanks to change camera view
        private float CheckDistanceBetweenTanks()
        {
            float maxDistance = 0.0f;

            for (int i = 0; i < m_Targets.Length - 1; i++)
            {
                
                for (int j = i + 1; j < m_Targets.Length; j++)
                {
                    float currDistance = Vector3.Distance(m_Targets[i].position, m_Targets[j].position);
